...
The boxplots graph shows there is heterogeneity across states, but very little heterogenetiy across years.
...
g.1 <- ggplot(df.transformed, aes(as.factor(perse), totfatrte)) + geom_boxplot(aes(fill = factor(perse))) + geom_jitter(width = 0.2) + ggtitle("Per Se Law and Fatality Rate Change for Top and Bottom States") + facet_wrap(~shortname) + theme(plot.title = element_text(size = 10, hjust = 0.5)) + theme(legend.position = "none") 

g.2 <- ggplot(df.transformed, aes(as.factor(bacat), totfatrte)) + geom_boxplot(aes(fill = factor(bacat))) + geom_jitter(width = 0.2) + ggtitle("Bac and Fatality Rate Change for Top and Bottom States") + facet_wrap(~shortname) + theme(plot.title = element_text(size = 10, hjust = 0.5)) + theme(legend.position = "none") + labs(y="")

grid.arrange(g.1, g.2, nrow = 1,  ncol = 2) 

g.1 <- ggplot(df.transformed, aes(as.factor(sball), totfatrte)) + geom_boxplot(aes(fill = factor(sball))) + geom_jitter(width = 0.2) + ggtitle("Fatality Rate Change for Top and Bottom States") + facet_wrap(~shortname) + theme(plot.title = element_text(size = 10, hjust = 0.5)) + theme(legend.position = "none") 

g.2 <- ggplot(df.transformed, aes(as.factor(gdl), totfatrte)) + geom_boxplot(aes(fill = factor(gdl))) + geom_jitter(width = 0.2) + ggtitle("Fatality Rate Change for Top and Bottom States") + facet_wrap(~shortname) + theme(plot.title = element_text(size = 10, hjust = 0.5)) + theme(legend.position = "none") + labs(y="")
...
We can see that in the years WY has lower fatality rate in the years they adopted the Per Se, Bac, and Seatbelt laws. They are yet to adopt the gdl law (perhaps they should!). RI shows a similar pattern, eventhough the fatality rate was already low. 
...
The variables *bac08, bac10, perse, sbprim, sbsecon, sl70plus, gdl* define the year in which the law has become effective. It ranges from 0-1 with decimals as the law could the change in the middle of the year. We don't want to transform this variable into binary as we might lose some important information about changes that happened in the middle of the year.

Variables like *perc14_24, unem, vehicmilespc* are already normalized and so no transformation is necessary.
...
Coefficients have gone down for the *bac08, bac10, perse, and sbprim* from -2.498, -1.418, -0.62, -0.07533 for the pooles OLS model to -1.437, -1.062, -1.152, -1.227 for the fixed effects model respectively. Furthermore *sbprim* has become statistically significant in the Fixed effects model.
...
Pooled OLS model assumes that the individual specific effects are uncorrelated with the independent variables. This is a wrong assumption as there will be correlation between different observations within the same state. But Fixed effects model assumes that the individual specific effect are correlated with the independent variables and therefore is more reasonable in the current context than the pooled OLS model.
